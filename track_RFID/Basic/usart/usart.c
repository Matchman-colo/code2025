...
volatile uint8_t flag_cnt_revceive = 0;		/* 为1时允许计时 */
...
// 2. 配置USART1引脚重映射（核心代码）
   GPIO_PinRemapConfig(GPIO_Remap_USART1, ENABLE);  // 部分重映射：TX->PB6, RX->PB7
	
	
	//USART1_TX   GPIOA.9	重定义成PB6
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6; //PA.9
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽输出
  GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化GPIOA.9
   
  //USART1_RX	  GPIOA.10初始化  重定义成PB7
