...
int D_AD_VALUE   = -35; 	//确定左右传感器差值
int LEFT_MAX     = 3244;   	//左传感器峰值
int RIGHT_MAX    = 3236;  	//右传感器峰值
int MID_MAX      = 3276;  		//中间传感器峰值
int LEFT_THERSH  = 2000;	//左传感器阈值
int RIGHT_THERSH = 2000;	//右传感器阈值
int LEFT_SPAN    = 4506;		//传感器向左移动跳跃差值   //790
int RIGHT_SPAN   = -4516;	//传感器向右移动跳跃差值   //1023 
...
int GetTraceData(void)
{
	...
	//printf("LA:%d MA:%d RA:%d\r\n",Left_AD,Mid_AD,Right_AD);
	//printf("DO1:%d ",Data_Out);
	...
	//printf("DO2: %d ",Data_Out);
	//printf("PV: %d PF: %d\r\n",PosFlagValue, PosFlag);
...
void GetParament(void)
{
	...
	Right_AD = -Get_Adc(ADC_Channel_5) + 4096;	  //左传感器获取的AD值
	Mid_AD   = -Get_Adc(ADC_Channel_4) + 4096;		//中间传感器获取的AD值
	Left_AD  = -Get_Adc(ADC_Channel_1) + 4096;		//右传感器获取的AD值
...
//		sprintf(buf, (const char* )"RightMax:%d ",RightMax);
