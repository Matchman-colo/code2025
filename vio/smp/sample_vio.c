...
HI_S32 SAMPLE_VIO_ViOnlineVpssOnlineRoute(HI_U32 u32VoIntfType)
{
    ...
    VPSS_CHN           VpssChnForGetFrame        = VPSS_CHN1;
    ...
    IVE_SRC_IMAGE_S stSrcImage; // 输入图像（YUV）
    IVE_DST_IMAGE_S stDstImage; // 输出图像（RGB）

    // 颜色空间转换控制参数
    IVE_CSC_CTRL_S stCscCtrl;
    stCscCtrl.enMode = IVE_CSC_MODE_PIC_BT601_YUV2RGB; // 设置 YUV 到 RGB 的转换模式

    // 用于 IVE 操作的句柄
    IVE_HANDLE hIveHandle;
    HI_BOOL bInstant = HI_TRUE; // 即时模式
    ...
    astVpssChnAttr[VpssChnForGetFrame].u32Width                    = stSize.u32Width;
    astVpssChnAttr[VpssChnForGetFrame].u32Height                   = stSize.u32Height;
    astVpssChnAttr[VpssChnForGetFrame].enChnMode                   = VPSS_CHN_MODE_USER;
    astVpssChnAttr[VpssChnForGetFrame].enCompressMode              = enCompressMode;
    astVpssChnAttr[VpssChnForGetFrame].enDynamicRange              = enDynamicRange;
    astVpssChnAttr[VpssChnForGetFrame].enVideoFormat               = enVideoFormat;
    astVpssChnAttr[VpssChnForGetFrame].enPixelFormat               = enPixFormat;
    astVpssChnAttr[VpssChnForGetFrame].stFrameRate.s32SrcFrameRate = 30;
    astVpssChnAttr[VpssChnForGetFrame].stFrameRate.s32DstFrameRate = 30;
    astVpssChnAttr[VpssChnForGetFrame].u32Depth                    = 1;
    astVpssChnAttr[VpssChnForGetFrame].bMirror                     = HI_FALSE;
    astVpssChnAttr[VpssChnForGetFrame].bFlip                       = HI_FALSE;
    astVpssChnAttr[VpssChnForGetFrame].stAspectRatio.enMode        = ASPECT_RATIO_NONE;
    ...
    abChnEnable[VpssChnForGetFrame]                     = HI_TRUE; 
    ...
    // 在调用 SAMPLE_COMM_VENC_Start 前打印分辨率
    printf("enPicSize: %d, Width: %d, Height: %d\n",enPicSize, stSize.u32Width, stSize.u32Height);
   ...
   s32Ret = SAMPLE_COMM_VENC_StartGetStream(VencChn, sizeof(VencChn)/sizeof(VENC_CHN));
    ...
    while(1)
     {
        s32Ret = HI_MPI_VPSS_GetChnFrame(VpssGrp, VpssChnForGetFrame, &stFrame, -1);
        HI_U32 u32FrameSize = stFrame.stVFrame.u32Width * stFrame.stVFrame.u32Height * 3 / 2;

        if (s32Ret == HI_SUCCESS)
        {
            if (stFrame.stVFrame.u64VirAddr[0] == 0)
            {
                // 检查物理地址是否有效
                if (stFrame.stVFrame.u64PhyAddr[0] != 0)
                {
                    // 映射物理地址到虚拟地址
                    HI_VOID* pVirAddr = HI_MPI_SYS_Mmap(stFrame.stVFrame.u64PhyAddr[0], u32FrameSize);
                    stFrame.stVFrame.u64VirAddr[0] = (HI_U64)(uintptr_t)pVirAddr;
                    pVirAddr = HI_MPI_SYS_Mmap(stFrame.stVFrame.u64PhyAddr[1], u32FrameSize);
                    stFrame.stVFrame.u64VirAddr[1] = (HI_U64)(uintptr_t)pVirAddr;
        ...
        memset(&stSrcImage, 0, sizeof(stSrcImage));
        stSrcImage.enType = IVE_IMAGE_TYPE_YUV420SP; // 输入图像类型为 YUV420SP
        stSrcImage.au64PhyAddr[0] = stFrame.stVFrame.u64PhyAddr[0]; // Y 分量物理地址
        stSrcImage.au64PhyAddr[1] = stFrame.stVFrame.u64PhyAddr[1]; // UV 分量物理地址
        stSrcImage.au64VirAddr[0] = stFrame.stVFrame.u64VirAddr[0]; // Y 分量虚拟地址
        stSrcImage.au64VirAddr[1] = stFrame.stVFrame.u64VirAddr[1]; // UV 分量虚拟地址
        stSrcImage.au32Stride[0] = stFrame.stVFrame.u32Stride[0]; // Y 分量的 stride
        stSrcImage.au32Stride[1] = stFrame.stVFrame.u32Stride[1]; // UV 分量的 stride
        stSrcImage.u32Width = stFrame.stVFrame.u32Width;
        stSrcImage.u32Height = stFrame.stVFrame.u32Height;

        // 初始化输出图像
        memset(&stDstImage, 0, sizeof(stDstImage));
        stDstImage.enType = IVE_IMAGE_TYPE_U8C3_PACKAGE; // 输出图像类型为 RGB888
        stDstImage.u32Width = stSrcImage.u32Width;
        stDstImage.u32Height = stSrcImage.u32Height;
        stDstImage.au32Stride[0] = stSrcImage.au32Stride[0]; // RGB 数据的 stride

        // 分配内存
        HI_MPI_SYS_MmzAlloc_Cached(&stDstImage.au64PhyAddr[0],(HI_VOID**)&stDstImage.au64VirAddr[0],"RGB Buffer",
            NULL,stDstImage.u32Height * stDstImage.au32Stride[0] * 3 );

        // 调用 IVE 的颜色空间转换函数
        HI_MPI_IVE_CSC(&hIveHandle, &stSrcImage, &stDstImage, &stCscCtrl, bInstant);
        ...
        if (HI_SUCCESS != s32Ret)
        {
            ...
            HI_MPI_SYS_MmzFree(stDstImage.au64PhyAddr[0], (HI_VOID*)(uintptr_t)stDstImage.au64VirAddr[0]);
            HI_MPI_VPSS_ReleaseChnFrame(VpssGrp, VpssChnForGetFrame, &stFrame);
           // continue;
        }

        void* imageData = stDstImage.au64VirAddr[0];
        int width = stDstImage.u32Width;
        int height = stDstImage.u32Height;

        // 调用灰度转换函数，并将图像保存到 /mnt 目录
        
        convertToGray(stDstImage.au64VirAddr[0], stDstImage.u32Width, stDstImage.u32Height, 3);
        ...
        {
            printf("Error(%#x), HI_MPI_VPSS_GetChnFrame failed!\n", s32Ret);
           // continue;
        }
...
HI_S32 SAMPLE_VIO_WDR_LDC_DIS_SPREAD(HI_U32 u32VoIntfType)
{
    ...
    u32BlkSize = VI_GetRawBufferSize(stSize.u32Width, stSize.u32Height, PIXEL_FORMAT_RGB_BAYER_16BPP, COMPRESS_MODE_SEG, DEFAULT_ALIGN);
    stVbConf.astCommPool[1].u64BlkSize  = u32BlkSize;
    ...
    /* 修改 VO 配置部分 */
    stVoConfig.enPicSize = enPicSize;  // 确保此处与 stSize 一致
     // 添加打印验证
    SAMPLE_PRT("VO 配置分辨率: width=%d, height=%d\n", stSize.u32Width, stSize.u32Height);
    /*start vo*/
    s32Ret = SAMPLE_COMM_VO_StartVO(&stVoConfig);
    if (HI_SUCCESS != s32Ret)
    {
